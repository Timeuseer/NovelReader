...
class update_books(wx.Frame):

    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_ANY, title=u"更新图书", pos=wx.DefaultPosition, size=wx.Size(434, 674),
        ...
        self.m_button231 = wx.Button(self, wx.ID_ANY, u"确认更改", wx.DefaultPosition, wx.DefaultSize, 0)
        bSizer34.Add(self.m_button231, 0, wx.ALL | wx.EXPAND, 5)
        ...
        self.m_button231.Bind(wx.EVT_BUTTON, self.up_sure)
        self.m_button24.Bind(wx.EVT_BUTTON, self.go_back)
        self.m_grid3.Bind(wx.grid.EVT_GRID_CELL_CHANGED, self.up_book)
    ...
    def up_sure(self, event):
        global up_info

        for x in range(7):
            up_info.append(self.m_grid3.GetCellValue(self.row, x))

        if self.m_grid3.GetCellValue(1, 0) != None:
            app = wx.App()

            main_win = serverui_upsure.updata_sure(None)
            main_win.Show()
            app.MainLoop()
